...
import numpy as np
import pandas as pd
from db._helpers import get_all_duckdb_archive_files
from db.data_store import DataStore



files = get_all_duckdb_archive_files()

for path in files:
    data_store = DataStore(db_path=path, readonly=True)

    # Fetch the data
    df = data_store.get_all_tick_data()
    df['timestamp'] = pd.to_datetime(df['timestamp']).astype(np.int64) * 10**3
    ...
    # Plot Bollinger bands on price ax
    fplt.plot(df['bb_upper'], ax=price_ax, color='#000')
    fplt.plot(df['bb_middle'], ax=price_ax, color='#333')
    fplt.plot(df['bb_lower'], ax=price_ax, color='#000')
