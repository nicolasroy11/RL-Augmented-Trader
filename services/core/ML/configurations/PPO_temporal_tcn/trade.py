...
from helpers import (
    cancel_all_orders, do_limit_buy, do_limit_sell, do_market_sell,
    get_balances_snapshot, get_instant_notional_minimum
)
...
from services.core.models import (
    FeatureSet, RunConfiguration, TickData, TickProbabilities,
    TradingSession, Transaction
)

# ⬇️ swap the env import to the TCN version
from services.core.ML.configurations.PPO_temporal_tcn.environment import (Environment)
...
class TraderRepository():
    ...
    def __init__(self):
        ...
        if runtime_settings.IS_TESTNET:
            ...
            if new_balances.free_base_balance < notional: 
                print('good to go')
    ...
    def run_single_buy_ppo_trader(self, feature_set_name: str) -> List[FullSingleLongCycleDto]:
        ...
        # ⬇️ point the run configuration at the TCN environment
    ...
    def run_loop(self):
        self.loop_number += 1
        latest_window_data = self.data_repo.get_latest_tick_window_data_by_run(
            num_ticks=HISTORY_WINDOW_LENGTH,
            run_id=self.data_repo.data_run.id
        )
        ...
        if balances.free_base_balance > 0 and self.current_buy is not None:
            ...
            print(
                f"Current price: {current_price:.2f} | Entry: {self.current_buy.strike_price:.2f} | "
                f"Pos: {self.current_buy.base_amount:.6f} | "
                f"Unrealized: {unrealized_pnl:.2f} | "
                f"Probs: {{ {', '.join([f'{k.name.lower()}: {round(v,4)}' for k,v in action_probs.items()])} }} | "
    ...
    def execute_trade(self, trade_signal: Actions, qty, price):
        ...
        # BUY -> open/increase long (maker limit)
