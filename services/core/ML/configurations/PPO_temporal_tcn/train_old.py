...
class RLRepository:
    ...
    def _run_ppo(self, num_episodes, gamma, gae_lambda, clip_epsilon, ppo_epochs, batch_size, ent_coef, lr) -> PPOTCNTrainingResults:
        ...
        for episode_number in range(num_episodes):
            ...
            for chunk in data_chunk_list:
                ...
                while not done:
                    ...
                    if step % 60:
                        env.current_step += 1
                        continue
                    if env.current_step >= len(env.data): done = True; continue
            ...
            # --- tensors ---
            rewards = torch.cat(reward_buf).squeeze(-1)
            values = torch.cat(value_buf).squeeze(-1)
            log_probs_old = torch.cat(logp_buf)
            ts_tensor = torch.stack(ts_buf, dim=0)
            state_tensor = torch.stack(state_buf, dim=0)
            actions_tensor = torch.cat(action_buf)
            dones = torch.cat(done_buf).squeeze(-1)
